...
> См. следующий вопрос для примеров неявных и явных курсоров.
:::

::: details Привести примеры явных и неявных курсоров

*Неявные* операторы вызываются всякий раз, когда в PL/SQL используется оператор ``SELECT``. Они могут использоваться только в операторах, которые возвращают не более одной строки. Неявные курсоры не требуют объявления и открытия, в то время как *явные* курсоры нужно объявить, открыть, считать и закрыть. Но явные курсоры позволяют обрабатывать запрос, возвращающий несколько строк.

**Пример неявного курсора:** получить максимальную зарплату среди сотрудников и записать её в переменную *макс_зарплата*.
```sql
SELECT MAX(зарплата)
INTO макс_зарплата
FROM сотрудники
...
**Пример явного курсора:** получить 5 самых наболее прослушиваемых песен.
```sql
DECLARE
   CURSOR топ_5_песен IS
   SELECT код_песни, название, кол_во_прослушиваний
   FROM песни
   ORDER BY кол_во_прослушиваний DESC
...
::: details Что такое последовательности (``SEQUENCE``)? Для чего они нужны и как работают?
:::

::: details Зачем нужен ``NOCACHE`` при создании последовательности и как скажется его отсутствие?
:::