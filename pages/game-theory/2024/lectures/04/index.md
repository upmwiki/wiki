...
где $i = \overline{1,n}$, $j = \overline{1, m}$, то средний выигрыш в этой игре при применении смешанной стратегии $\bar{x} = [\xi_1, ..., \xi_n]$ первым игроком и $\bar{y} = [\eta_1, ..., \eta_m]$ вторым игроком будет определяться выражением
