...
$$
где $m = \max \set{ n_1, n2 }$, $\tilde{P}$ и $\tilde{Q}$ — многочлены степени $m$, $k = \begin{cases} \text{кратность корня} ~ re^{i\varphi}, & \text{если} ~ re^{i\varphi} = \rho e^{i\alpha}, \\ 0, & \text{в противном случае}. \end{cases}$
...
2. Частное неоднородное:
   $$ y_{n+1}^\text{чн} + 2y_n^\text{чн} + y_{n-1}^\text{чн} = (-1)^k; $$
   $$ y_n^\text{чн} = An^2 (-1)^n, $$
...
Подставим данное реение в исходное уравнение:
