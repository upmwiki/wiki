...
2. сумма всех элементов: $||\bar{x}||_2 = \sum_{i=1}^n |x_i|$;
...
Множесво конечномерных векторов, на котором определена норма, называется **линейным нормированным пространством**. В таком пространстве определяется метрика (расстояние) между векторами:
...
Рассмотрим линейное нормаированное пространство векторов $H_n$ ($\bar{x} \in H_n$, $n$ — размерность векторов). Рассмотрим также квадратную матрицу $A = (a_{ij})_{n \times n}$.
...
Ограниченнй и аддитивный (допускающий сложение и умножение на число) оператор будем называть **линейным**.
...
Отсюда следуют свойства норм операторов. Пусть $A$, $B$, $C$ — некие константы, тогда:
...
2. $||A|| _ 2 = \underset{j}{\max} \sum _ {i=1}^n |a _ {ij}|$<br>*(максимальная сумма модулей по стробцу)*
